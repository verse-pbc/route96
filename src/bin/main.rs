...
async fn main() -> Result<(), Error> {
    // Initialize logging - respect RUST_LOG if set, otherwise apply defaults.
    if std::env::var("RUST_LOG").is_err() {
        std::env::set_var(
            "RUST_LOG",
            "route96=debug,rocket::launch=info,rocket::server::health=off,rocket::server=info,rocket=info",
        );
    }
